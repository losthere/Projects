 ...
 */
 ...
 .factory('uitkDynamicTableExporter', function() {
 	...
 	return function (table, fileName) {
 		...
 		['*[class^="cux-icon-"]', '.oui-a11y-hidden','input', 'textarea', 'button', 'img'].forEach(function(selector){
    ...
    // ===================================================
 		...
 		// for separating phone and fax into separate columns
 		var phoneFaxIdx = -1;
 		for (i = 0; i < headerArray.length; i++) {
 			if (headerArray[i].indexOf('Phone') > -1 && headerArray[i].indexOf('Fax') > -1) {
 				phoneFaxIdx = i;
 				headerArray.splice(i, 1);
 				headerArray.splice(i, 1);
 				headerArray.splice(i, 0, 'Phone', 'Fax');
 				break;
 			}
 		}
 		...
 		for ( i = 0; i < tableRow.length; i++ ) {
 			...
 			if ( rowAttribute !== null && rowAttribute === "row" ) {
 				...
 				for ( j = 0; j < tableCell.length; j++ ) {
 					...
 					//to export select dropdown values in assignments
 					if(tableCellValues.length === 0){
 						tableCellValues = $(tableCell[j]).find('select').find('option:selected');
 						//to export anchor tag values
 						if(tableCellValues.length === 0){
 							tableCellValues = $(tableCell[j]).find('a');	
 						}
 					}
 			...
 			if(rowAttribute !== null ) {
 				...
 				if (cellArray.length > 0) {
 			 		// for separating phone and fax into separate columns
 					if (phoneFaxIdx > -1) {
 						var phoneFaxCellText = cellArray[phoneFaxIdx];
 						phoneFaxCellText = phoneFaxCellText.substring(1, phoneFaxCellText.length - 1);// remove multiple " " in the string
 						cellArray[phoneFaxIdx] = phoneFaxCellText.trim().replace(/\s\s+/g, ' ');// remove multiple spaces

 						var phoneFaxValues = cellArray[phoneFaxIdx].split(" ");
 						cellArray.splice(phoneFaxIdx, 1);
 						cellArray.splice(phoneFaxIdx, 0, phoneFaxValues[0], phoneFaxValues[1]);
 					}
